...
class ResidualAttentionBlock_IAPT(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
                 text_layer=False, i=0, design_details=None):
        super().__init__()
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        elif current_trainer == 'IAPT' or current_trainer == 'MPT':
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_IAPT(width, heads, attn_mask, True,
                                                                         text_layer, i,
                                                                         design_details) if prompts_needed > i
                                             else ResidualAttentionBlock_IAPT(width, heads, attn_mask, False,
                                                                              text_layer, i, design_details)
                                             for i in range(layers)])
        else:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,return_all: bool = False):
        ...
        # 注意：patch token 永远取 cls 后面的前 num_patches 个
        # 这样就不会把 prompt token 混进去
        global_feat = self.ln_post(x[:, 0, :])  # [B, width]
        patch_feat = self.ln_post(x[:, 1:1 + num_patches, :])  # [B, num_patches, width]

        if self.proj is not None:
            global_feat = global_feat @ self.proj  # [B, output_dim]
            patch_feat = patch_feat @ self.proj  # [B, num_patches, output_dim]
...
class VisionTransformer_IAPT(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
                 output_dim: int, design_details):
        super().__init__()
        self.input_resolution = input_resolution
    ...
    def forward(self, x: torch.Tensor, img_prompts, cross_prompts_visual_deeper):
        ...
        outputs = self.transformer([x, cross_prompts_visual_deeper])  # 这里的topk是LBD
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe":
                ...
                self.visual = VisionTransformer_IAPT(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,

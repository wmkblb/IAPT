...
def extend_cfg(cfg):
    ...
    # Config for IAPT
    cfg.TRAINER.IAPT = CN()
    cfg.TRAINER.IAPT.N_CTX = 2  # number of context vectors
    cfg.TRAINER.IAPT.CROSS_LAYER = 6  # cross layer
    cfg.TRAINER.IAPT.CTX_INIT = "a photo of a"  # initialization words (only for language prompts)
    cfg.TRAINER.IAPT.PREC = "fp16"
    cfg.TRAINER.IAPT.PROMPT_DEPTH = 9  # Max 12, minimum 0, for 1 it will act as shallow IAPT (J=1)
    cfg.TRAINER.IAPT.TEACHER_NAME = "ViT-L/14"
    cfg.TRAINER.IAPT.LAMBD = 10.
    cfg.DATASET.SUBSAMPLE_CLASSES = "all"  # all, base or new
